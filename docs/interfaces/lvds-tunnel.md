...
### 1.5 The companion side — OPEN

The FMC LPC `LA` pair → `LIFCL-40-9BG400C` ball mapping, and which of
those pairs land on the LIFCL-40's **bottom** bank (the only bank with
true LVDS output, per PLAN.md A.0's datasheet check), **is not yet
recorded anywhere in this repo.** It must be read out of the LIFCL-40-EVN
board schematic / user guide and written into §1 of this document before
companion RTL starts. Tracked as open item **O1** in §11.

The requirement to satisfy when picking pairs:

- The companion's **transmit** pair (→ Kuntur `tun_rx`, G9/F9) must be a
  true-LVDS-output-capable bottom-bank pair.
- The companion's **receive** pair (← Kuntur `tun_tx`, E9/E8) needs only
  LVDS input, available on more banks — pick it for routing convenience
  and pair-length matching against the transmit pair.
- Both must be `LA` pairs exposed by the IAM breakout at 1.27 mm pitch.
...
| ID | Item | Gates | Owner |
|---|---|---|---|
| **O1** | **FMC LPC `LA` pair → LIFCL-40 ball mapping.** Not recorded anywhere in this repo; must come from the LIFCL-40-EVN board documentation and be written into §1.5. Under decision 4 this now carries an extra requirement: the pair receiving **`TUN_CLK` must be a clock-capable input** able to drive a PLL and the edge-clock network. The `TUN_DATA` pair needs only LVDS input. | All companion RTL, and the pigtail wiring | Manuel |
...
1. **Resolve O1** — a documentation lookup against the LIFCL-40-EVN board
   docs, needing no hardware powered, gating all companion RTL and the
   pigtail wiring. Note the new clock-capable-input requirement.
