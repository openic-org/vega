...
def _run_wire_format_checks():
    # Slot→ch_a mapping must be injective over all 33 sampling slots and fit
    # the 6-bit field, whatever SLOT_OFFSET/FRAME_SLOTS are set to. This is
    # the check that survives a change to those constants; the rung values
    # below cannot test the constant itself, since the FakeReader inverts
    # the same one.
